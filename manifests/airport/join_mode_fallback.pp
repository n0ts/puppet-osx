# Public: Set What to Do When None of the Preferred Networks Are Available
# Modes: Prompt | JoinOpen | KeepLooking | DoNothing
class osx::airport::join_mode_fallback($mode = 'DoNothing') {
  include osx::airport

  if $mode !~ /^(Prompt|JoinOpen|KeepLooking|DoNothing)/ {
    fail("osx::airdrop::join_mode_fallback[mode] must be one of the following: Prompt, JoinOpen, KeepLooking, DoNothing; is ${mode}")
  }
