# Public: Set the top left hot corner to action
class osx::dock::hot_corners_top_left(
  $only_with_modifier = false,
  $action = 'do nothing') {

  $corner_abbreviation = 'tl'
  case $action {
  ...
  boxen::osx_defaults { "Set the top left hot corner to ${action}":
    user   => $::boxen_user,
  ...
  boxen::osx_defaults { "Set the modifier for the top left hot corner to ${only_with_modifier}":
    user   => $::boxen_user,
