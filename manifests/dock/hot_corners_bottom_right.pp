# Public: Set the bottom right hot corner to action
class osx::dock::hot_corners_bottom_right(
  $only_with_modifier = false,
  $action = 'do nothing') {

  $corner_abbreviation = 'br'
  case $action {
  ...
  boxen::osx_defaults { "Set the right hot corner to ${action}":
    user   => $::boxen_user,
  ...
  boxen::osx_defaults { "Set the modifier for the right hot corner to ${only_with_modifier}":
    user   => $::boxen_user,
